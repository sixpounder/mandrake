...
module.exports = function tree(router, options, cb) {
  ...
  m.on('file', function(f) {
    ...
    if(options.log === true) {
      console.log(`${chalk.cyan("mandrake")} - mount GET ${options.mountPoint}${normalizedPath} ${chalk.green("ok")}`);
    }

    router.get(`${options.mountPoint}${normalizedPath}`, htmlRenderer(structure, contentIndex, options));
